...
CDXC      i = I
CDXCV     REM
D         REM Declare 2 numbers, increment each in inner + outer loop
DV        REM and check password corresponding to them. goto end if done
DX        REM to print the password.
DXV       PRINT "trying complex matches for user " + username + CHR(X)
DXX       DIM decimals(X) AS STRING
DXXV      decimals(I) = "0"
DXXX      decimals(II) = "1"
DXXXV     decimals(III) = "2"
DXL       decimals(IV) = "3"
DXLV      decimals(V) = "4"
DL        decimals(VI) = "5"
DLV       decimals(VII) = "6"
...
DLXV      decimals(IX) = "8"
DLXX      decimals(X) = "9"
DLXXV     DIM j AS INTEGER
DLXXX     DIM k AS INTEGER
DLXXXV    j = I
DXC       k = I
DXCV      IF CHECKPASS(username, words(i) + decimals(j) + decimals(k)) THEN GOTO DCLXXX
DC        j = j + I
DCV       IF j < XI THEN GOTO DXCV
DCX       j = I
DCXV      k = k + I
DCXX      IF k < XI THEN GOTO DXCV
DCXXV     i = i + I
DCXXX     IF i > pwdcount THEN GOTO DCLV
DCXXXV    GOTO DLXXXV
DCXL      REM
DCXLV     REM We failed to find a password like this so give up:
DCL       REM
DCLV      PRINT "no complex matches for user " + username + CHR(X)
DCLX      END
DCLXV     REM
DCLXX     REM We succeeded at finding a complex PW so print it:
DCLXXV    REM
DCLXXX    PRINT "found complex match!! for user " + username + CHR(X)
DCLXXXV   PRINT "password: " + words(i) + decimals(j) + decimals(k) + CHR(X)
DCXC      END
